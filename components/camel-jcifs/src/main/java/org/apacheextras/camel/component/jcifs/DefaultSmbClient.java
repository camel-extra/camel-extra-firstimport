...
public class DefaultSmbClient implements SmbClient {
     ...
     *
     * @param smbApiFactory a {@link SmbApiFactory} to use instead of the
    ...
    public List<SmbFile> listFiles(String url) throws IOException {
        ...
        try {
            ...
            if (elements != null && elements.length > 0 && elements[0] != null && "jcifs.smb.Dfs".equals(elements[0].getClassName())
                && "resolve".equals(elements[0].getMethodName())) {
                LOGGER.warn("Ignoring NPE in jcifs.smb.Dfs.resolve: {}", ex.getMessage());
            } else {
                throw ex;
            }
        }
        ...
        try {
            if (sFile.exists()) {
                if (renamedFile.exists()) {
                    throw new IOException("Could not rename source file '" + sFile.getName() + "' since target name '" + renamedFile.getName() + "' already exists.");
                }
